...
music.addEventListener('timeupdate', ()=>{
    ...
    let second = Math.floor(music_dur%60)

    if(second<10){
        second = `0${second}`;
    }
    currentEnd.innerText = `${minute}:${second}`;

    let minute_1 = Math.floor(music_curr/60);
    let second_1 = Math.floor(music_curr%60)

    if(second_1<10){
        second_1 = `0${second_1}`;
    }
    currentStart.innerText = `${minute_1}:${second_1}`;
